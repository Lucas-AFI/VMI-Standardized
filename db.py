...
Machine-specific environment variables:
    SQL_SERVER_NAME : SQL Server instance name (e.g. MATRIX1\SQLEXPRESS)
    SQL_DB_NAME     : Database name (e.g. ATTC, SunHyd, Fillauer)
    SUPPLIER_KEY    : AFI supplier key in Matrix (default: 1 if not set)
"""
...
SUPPLIER_KEY = int(environ.get('SUPPLIER_KEY', 1))
...
def connect_db():
    ...
    for i in l_drivers:
        try:
            l_conn = connect(
                'Driver=' + i + ';'
                'Server=' + environ['SQL_SERVER_NAME'] + ';'
                'Database=' + environ['SQL_DB_NAME'] + ';'
